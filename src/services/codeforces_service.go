...
func (s *CodeforcesService) Url() string {
	return "https://codeforces.com/api/contest.list"
}

func (s *CodeforcesService) FetchUpcomingContests() <-chan models.ContestDto {
	...
	go func() {
		...
		for _, contest := range data.Contests {
			...
			contestUrl := fmt.Sprintf("https://codeforces.com/contests/%d", contest.Id)
